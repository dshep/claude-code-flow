...
mcpCommand
  ...
  .action(async (options: any) => {
    try {
      ...
      console.error(chalk.red(`❌ Failed to start MCP server: ${getErrorMessage(error)}`));
      process.exit(1);
    }
  });
...
mcpCommand
  ...
  .action(async () => {
    try {
      ...
      console.error(chalk.red(`❌ Failed to stop MCP server: ${getErrorMessage(error)}`));
      process.exit(1);
    }
  });
...
mcpCommand
  ...
  .action(async () => {
    try {
      ...
      console.error(chalk.red(`❌ Failed to get MCP status: ${getErrorMessage(error)}`));
    }
  });
...
mcpCommand
  ...
  .action(async () => {
    try {
      ...
      console.error(chalk.red(`❌ Failed to show MCP config: ${getErrorMessage(error)}`));
    }
  });
...
mcpCommand
  ...
  .action(async () => {
    try {
      ...
      console.error(chalk.red(`❌ Failed to restart MCP server: ${getErrorMessage(error)}`));
      process.exit(1);
    }
  });
